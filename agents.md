...
- Respect the tier map when changing navigation or hierarchy. `Dashboard`, `Capture`, `Thoughts`, `Promises`, and `Reports` are the calm workspace loop; `Projects`, `Activities`, `Skills`, and `Connections` are deeper structured surfaces; `Admin`, `Processes`, and `Apps` belong to system/operator depth.
