...
- Notes:
  ...
  - admin page supports adding magnet candidates, resolving metadata, queueing downloads, inspecting progress, retrying failures, editing titles, setting thumbnails, and deleting local files while preserving records
  ...
  - preserves evicted movie records while clearing local file paths and cache state, so magnet-backed items can be downloaded again from the UI
  - stores generated posters under `RTV_MEDIA_DIR/posters` so thumbnails survive local movie-file deletion
